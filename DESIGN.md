...
我理解的核心需求是：
1、基础业务管理：产品、客户、持仓、跟进记录的增删改查
2、AI智能问答：通过自然语言查询数据库，生成图表，解答金融问题
3、可视化展示：前端页面展示数据及图表
...
1、代码生成：生成基础 CRUD 模板代码，通用代码生成。
2、SQL 生成：将自然语言转为 SQL 查询。
3、意图识别：判断是否需要生成图表。
4、回答生成：将查询结果转为自然语言。
...
1. 快速生成基础代码 - Express 路由、React 组件模板，减少重复劳动
2. SQL 语句生成 - 复杂多表联查的 SQL 编写，LLM 表现出色
3. 思路梳理 - 实现方案不确定时，AI 能提供多个可行方向

3.4 AI 帮不上忙的地方

1. 业务逻辑理解 - 基金行业的专业术语、业务规则需要人工确认
2. 异常边界处理 - AI 不了解生产环境的特殊情况（如数据库连接超时）
3. 调试定位问题 - 线上 bug 仍需人工分析日志、定位根因
4. 安全性判断 - SQL 注入风险需要人工二次审核
...
1、无登录验证
2、AI 回复不稳定，同一问题多次提问，回复可能不同
3、 SQL 注入风险
4、无请求限流，AI 接口可能被恶意调用
